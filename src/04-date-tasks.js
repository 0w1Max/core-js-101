...
function timeSpanToString(startDate, endDate) {
  ...
  return `${isLessThanTen(hours)}:${isLessThanTen(minutes)}:${isLessThanTen(seconds)}.${milliseconds}`;
}
...
function angleBetweenClockHands(date) {
  ...
  const ang = Math.abs((hours + (minutes / 60)) * 30 - (minutes * 6));
